...
import { deleted, selectedColor, toggled } from "../redux/todo/actions";

const Todo = ({ id, completed, text, color }: TodoTypes) => {
  const dispatch = useDispatch();
  const handleStateChange = (todoId: number) => {
    dispatch(toggled(todoId));
  };
  const handleColorChange = (todoId: number, color: string) => {
    dispatch(selectedColor(color, todoId));
  };
